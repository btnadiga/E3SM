...
variables = ('zeta_i','gradThdiff_i','Va_gTh_i','zeta_j','gradThdiff_j','Va_gTh_j','zeta_k','gradTh_k','Va_gTh_k','u','Th','Va_gTh')
...
ilon0 = np.where(lonmod == lon0)                                                                       
Thpi  = ds.Th.isel(time=time[tframe],lon=ilon0[0][0]+1)                                                
gradThdiff_i = (Thpi - Th) 

for ilat in np.arange(0,len(latmod)):                                                                  
    dx = rearth * np.cos(latmod[ilat] * np.pi / 180) * (lonmod[ilon0[0][0] + 1] - lonmod[ilon0[0][0]])\
         * np.pi / 180                                                                                                   
    gradThdiff_i[:,ilat] = gradThdiff_i[:,ilat] / np.asscalar(dx)                                      

gradThdiff_j = np.asarray(Th[:,1:]) - np.asarray(Th[:,0:-1])                                           

for ilat in np.arange(0,len(latmod)-1):                                                                
    dy = rearth * np.pi * (latmod[ilat+1] - latmod[ilat]) / 180                                        
    gradThdiff_j[:,ilat] = gradThdiff_j[:,ilat] / np.asscalar(dy)                                      

gradThdiff_j_end = gradThdiff_j[:,-1]                                                                  
gradThdiff_j_end.shape = (len(gradThdiff_j_end),1)                                                     
gradThdiff_j = np.concatenate([gradThdiff_j,gradThdiff_j_end],axis=1)          
